...
public class LocationController {
    ...
    public LocationController() {
        ...
        knownLocations.add(new Location("Wien", 48.2082, 16.3738));
    ...
    @GetMapping("/location")
    public Location location(@RequestParam(value = "name", defaultValue = "") String name) {
       for(int i = 0; i < knownLocations.size(); i++) {
           if(knownLocations.get(i).name.equalsIgnoreCase(name)) {
